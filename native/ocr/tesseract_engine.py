...
def image_to_text(image: Image.Image, text_orientation: str = "horizontal") -> str:
    """Run Tesseract OCR on a PIL Image using OCRCONFIG settings."""
    pytesseract.pytesseract.tesseract_cmd = str(paths.tesseract_exe_path())
    ...
    custom_config = (
        f'--tessdata-dir "{paths.tessdata_dir()}" --oem {oem} --psm {psm} '
        f'-c preserve_interword_spaces=1 {extra_options}'
    )
